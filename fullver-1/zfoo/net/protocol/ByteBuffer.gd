...
func setWriteOffset(writeIndex: int):
	if (writeIndex > buffer.get_size()):
		Log.error("writeIndex[{}] out of bounds exception: readOffset: {}, writeOffset: {} (expected: 0 <= readOffset <= writeOffset <= capacity: {})", writeIndex, readOffset, writeOffset, buffer.get_size())
		return
...
func setReadOffset(readIndex: int):
	if (readIndex > writeOffset):
		Log.error("readIndex[{}] out of bounds exception: readOffset: {}, writeOffset: {} (expected: 0 <= readOffset <= writeOffset <= capacity: {})", readIndex, readOffset, writeOffset, buffer.size())
		return
...
func writeInt(value):
	if !(minInt <= value && value <= maxInt):
		Log.error("value must range between minInt:-2147483648 and maxInt:2147483647")
		return
	writeLong(value)
	pass
