...
static func write(buffer: ByteBuffer, packet: SignalAttachment):
	if (packet == null):
		buffer.writeInt(0)
		return
	buffer.writeInt(-1)
...
static func read(buffer: ByteBuffer) -> SignalAttachment:
	var length = buffer.readInt()
	if (length == 0):
		return null
	var beforeReadIndex = buffer.getReadOffset()
	var packet: SignalAttachment = SignalAttachment.new()
